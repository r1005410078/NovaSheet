...
describe('VisibleFormatResolver', () => {
  it('cellFormats 收集可见区命中的格式（view 坐标）', () => {
    ...
    formatStore.apply({ startRow: 1, endRow: 1, startCol: 0, endCol: 0 }, { fillColor: '#fff2cc' })
  ...
  it('mergeRegions 返回与可见区相交的合并（view 坐标）', () => {
    ...
    mergeStore.merge({ startRow: 0, endRow: 1, startCol: 0, endCol: 1 })
  ...
  it('anchor 滚出扫描范围但区域相交时补发 anchor 格式', () => {
    ...
    formatStore.apply({ startRow: 0, endRow: 0, startCol: 0, endCol: 0 }, { fillColor: '#d9ead3' })
    mergeStore.merge({ startRow: 0, endRow: 2, startCol: 0, endCol: 0 })
