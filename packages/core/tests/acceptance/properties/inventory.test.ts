...
describe('Core acceptance properties', () => {
  ...
  it('core.L0.cell-type-store-raw-remap stores, clears, restores, and remaps raw cell type overrides', () => {
    ...
    const rowDeleteStore = new CellTypeStore()
    rowDeleteStore.set(asRawRange({ startRow: 1, endRow: 1, startCol: 1, endCol: 1 }), 'date')
    rowDeleteStore.set(asRawRange({ startRow: 2, endRow: 2, startCol: 1, endCol: 1 }), 'checkbox')
    rowDeleteStore.remapAfterRowsDeleted([2])
    expect(rowDeleteStore.get(2, 1)).toBeUndefined()
    expect(rowDeleteStore.get(1, 1)).toBe('date')

    const rowMapStore = new CellTypeStore()
    rowMapStore.set(asRawRange({ startRow: 1, endRow: 1, startCol: 1, endCol: 1 }), 'date')
    rowMapStore.set(asRawRange({ startRow: 2, endRow: 2, startCol: 1, endCol: 1 }), 'checkbox')
    rowMapStore.remapByRowIndexMap(new Map([[1, 4]]))
    expect(rowMapStore.get(4, 1)).toBe('date')
    expect(rowMapStore.get(2, 1)).toBeUndefined()
    ...
    const colMapStore = new CellTypeStore()
    colMapStore.set(asRawRange({ startRow: 3, endRow: 3, startCol: 1, endCol: 1 }), 'date')
    colMapStore.set(asRawRange({ startRow: 3, endRow: 3, startCol: 2, endCol: 2 }), 'checkbox')
    colMapStore.remapByColIndexMap(new Map([[1, 4]]))
    expect(colMapStore.get(3, 4)).toBe('date')
    expect(colMapStore.get(3, 2)).toBeUndefined()
  })
})
