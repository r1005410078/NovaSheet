...
**渲染后端依赖反转**：`@zhiguang/novasheet-canvas2d` 实现 `ports/RenderBackend`、反向依赖 core；core `src/` 永不 import canvas2d。组合根（storybook）`new Grid({ data, backend: canvas2dBackend })` 注入。

**外部包** 只从 `@zhiguang/novasheet-core` 的 `index.ts` 按名导入。
