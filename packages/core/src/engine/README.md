...
| `features/format/` | 格式 store + FormatState + FormatController/EventHandler/Undo + VisibleFormatResolver |
| `features/merge/` | MergeStore + MergeViewResolver |
...
6. 抽离 layout state 初始化与 rebuild 规则。✅（已完成，`DefaultLayoutState` 聚合根）
7. 抽离 format/merge mutation 协调逻辑。✅（已完成，`DefaultFormatState` 聚合根）
