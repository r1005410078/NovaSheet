...
| 5 | 抽离 undo replay | 🟡 | M1 已落地：`UndoRegistry`/`UndoReplay`/`CellUndoHandler` 接线，`undo()/redo()` 委派 `UndoReplay`，`editCell/clearRange/paste` 走 registry，余 18 kind dual-track 回退旧 switch（M2 format/merge → M3 单域结构 → M4 复合 + 删旧 switch）。 |
...
下一步候选：undo M2（`FormatUndoHandler`：format/merge/unmerge 迁出旧 switch），续推第 5 步。
