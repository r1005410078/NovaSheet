    ...
    return [
      {
        ...
        iconColor: activeDirection === 'asc' ? 'var(--ns-menu-accent)' : undefined,
        category: 'sort',
      ...
      },
      {
        ...
        iconColor: activeDirection === 'desc' ? 'var(--ns-menu-accent)' : undefined,
        category: 'sort',
...
class SortedDataSource implements DataSource {
  ...
  private handleUpstreamEvent(event: DataSourceEvent): void {
    ...
    // Phase 4.5：行插入/删除/行数变化只更新本层 order，不触发 pipeline.rebuild
