...
- **包**：`@zhiguang/novasheet-core` · `@zhiguang/novasheet-canvas2d` · `@zhiguang/novasheet-react` · `@zhiguang/novasheet-cell-kit` · `@zhiguang/novasheet-mbd`
- **对外入口**：`import { Grid } from '@zhiguang/novasheet-core'`，渲染后端经 `GridOptions.backend` 注入（`canvas2dBackend()`）；React 场景用 `@zhiguang/novasheet-react` 的 `<NovaExcel />` / `<NovaSheetGrid />`
...
flowchart TB
  subgraph Apps["组合根（应用层）"]
    ...
    React["@zhiguang/novasheet-react<br/>NovaExcel / NovaSheetGrid / hooks"]
  end

  subgraph CoreP["@zhiguang/novasheet-core — 引擎 + DOM 壳 + 公开 facade"]
  ...
  subgraph C2D["@zhiguang/novasheet-canvas2d — Canvas2D 渲染后端"]
    Backend["canvas2dBackend()<br/>RenderBackendFactory"]
    Renderer["Canvas2DRenderer + painters + HighDPI"]
    Backend --> Renderer
  end

  CellKit["@zhiguang/novasheet-cell-kit<br/>opt-in 单元格组件（rich-text）"]
  MBD["@zhiguang/novasheet-mbd<br/>MD 场景校验工具（dev-only）"]
...
| 包                    | 职责                                                                                                                                                                             | 不含                  |
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- |
| `@zhiguang/novasheet-core`     | 全部状态与行为：数据、几何、选区、编辑、格式、合并、fill、剪贴板、undo/redo、校验 + DOM 壳（host/scroll/interaction/overlay/runtime）+ 公开 `Grid` facade + `RenderBackend` 端口 | 任何 Canvas 绘制      |
| `@zhiguang/novasheet-canvas2d` | `RenderBackend` 唯一 shipped 实现：`Canvas2DRenderer`、painters、HighDPI、`cellRenderers` 自定义绘制注册                                                                         | 引擎状态、DOM 编排    |
| `@zhiguang/novasheet-react`    | React 适配：`<NovaExcel />` Excel 壳、`<NovaSheetGrid />`、hooks、toolbar；一切能力最终调用公开 `Grid` 方法                                                                      | 引擎/渲染逻辑         |
| `@zhiguang/novasheet-cell-kit` | 第一方 opt-in 单元格组件（首个 rich-text：codec + canvas renderer + inline 编辑器 + React toolbar），与第三方扩展走同一注册路径                                                  | 默认打包进 core/react |
| `@zhiguang/novasheet-mbd`      | BDD 场景 MD 的 `validate`/`manifest` 工具链（dev-only）                                                                                                                          | 运行时代码            |
...
- `@zhiguang/novasheet-canvas2d` 侧：`Canvas2DRenderer.render(frame)` 只读 frame 绘制；painters：`CellPainter`、`HeaderPainter`、`RowHeaderPainter`、`GridLinesPainter`、`FormatFillPainter`、`FormatBorderPainter`、`EmptyStatePainter`；HighDPI 位图管理在 `surface/`。
...
| 单元格扩展 API：`cellTypes` / `cellEditors` / `cellAttachments` / `cellRenderers`（backend 侧）+ per-cell `setCellType` override | ✅（参考实现 `@zhiguang/novasheet-cell-kit` rich-text） |
| 数据校验（sync/async `ValidatorDefinition`，全写入路径自动接线）                                                                 | ✅                                             |
| 远程滑动窗口数据源 `WindowedDataSource`                                                                                          | ✅                                             |
| React 适配 `@zhiguang/novasheet-react`（NovaExcel 壳 + hooks）                                                                            | ✅                                             |
...
- 引擎、类型与数据：`@zhiguang/novasheet-core`（[README](../packages/core/README.md)）
- Canvas2D 后端与自定义 painter：`@zhiguang/novasheet-canvas2d`
- React 组件：`@zhiguang/novasheet-react`（[README](../packages/react/README.md)）
