...
最近交付：**Phase 5-C 值格式化 · 单元格扩展 API（+ `@zhiguang/novasheet-cell-kit` 富文本）· 数据校验 · `WindowedDataSource` 远程数据 · React 适配**。下一里程碑：**Phase 5-D 条件格式**。
...
| 公共 API                 | `import { Grid } from '@zhiguang/novasheet-core'` + `backend: canvas2dBackend()` 注入渲染后端；React 用 `@zhiguang/novasheet-react` |
| License                  | MIT                                                                                                               |
...
| 单元格扩展 API                     | `cellTypes` / `cellEditors` / `cellAttachments`（core）+ `cellRenderers`（backend）四轴注册 · per-cell `setCellType` override · `@zhiguang/novasheet-cell-kit` 富文本参考实现              | ✅     | [spec](docs/superpowers/specs/2026-06-12-novasheet-cell-extension-api-design.md) · [override](docs/superpowers/specs/2026-06-14-novasheet-cell-level-type-override-design.md)      |
...
| React 适配                         | `@zhiguang/novasheet-react`：`<NovaExcel />` Excel 壳 · `<NovaSheetGrid />` · hooks · toolbar                                                                                              | ✅     | [README](packages/react/README.md)                                                                                                                                                 |
...
```
┌─────────────────────────────────────────────────────────────┐
│   组合根：apps/storybook · @zhiguang/novasheet-react                 │
...
novasheet/
├── packages/
│   ├── core/                @zhiguang/novasheet-core — 引擎 + DOM 壳 + 公开 Grid facade
│   ├── canvas2d/            @zhiguang/novasheet-canvas2d — Canvas2D 渲染后端（RenderBackend 实现）
│   ├── react/               @zhiguang/novasheet-react — React 适配（NovaExcel 壳 + hooks）
│   ├── cell-kit/            @zhiguang/novasheet-cell-kit — opt-in 单元格组件（富文本）
│   └── mbd/                 @zhiguang/novasheet-mbd — MD 场景 BDD 工具链（dev-only）
├── apps/
│   └── storybook/           组件变体玩具间
├── docs/
