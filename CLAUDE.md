...
3. **Theme is the ONLY source of visual values.** No hardcoded px, fonts, or color literals in `packages/canvas2d/src/painters/` or `render/`. Future ESLint custom rule planned.
...
| global-stub helper                | `packages/web/tests/helpers/global-stub.ts` (+ duplicate in canvas2d) |
